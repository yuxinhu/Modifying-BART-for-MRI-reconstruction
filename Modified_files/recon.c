...
{
	long img_dims[DIMS];
	md_select_dims(DIMS, ~COIL_FLAG, img_dims, dims);	 //yuxinh, generate img_dims
	sense_recon2(conf, dims, img_dims,image, sense_op, pat_dims, pattern,
		iter2_call_iter, &(struct iter_call_s){ { }, italgo, iconf },
		     1, MAKE_ARRAY(thresh_op), NULL, ksp_dims, kspace, image_truth, precond_op);
}

// yuxinh
void sense_recon2(const struct sense_conf* conf, const long dims[DIMS], 
