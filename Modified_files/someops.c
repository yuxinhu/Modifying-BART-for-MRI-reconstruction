...
static void yuxinT_free_data(const linop_data_t* _data)
{
	const struct yuxinT_data* data = CONTAINER_OF(_data, const struct yuxinT_data, base);
	free((void*)data);
}


static void yuxinT0_apply(const linop_data_t* _data, complex float* dst, const complex float* src)
{
	const struct yuxinT_data* data = CONTAINER_OF(_data, const struct yuxinT_data, base);
	long pos[DIMS];
	memset(pos, 0, DIMS * sizeof(long));
	pos[10] = 1;
	md_copy_block(DIMS, pos, data->img_dims2, dst, data->img_dims, src, CFL_SIZE);
	md_zadd2(DIMS, data->img_dims2, data->strs_img2, dst, data->strs_img1, src, data->strs_img2, dst);
}

static void yuxinT0_adjoint(const linop_data_t* _data, complex float* dst, const complex float* src)
{
	const struct yuxinT_data* data = CONTAINER_OF(_data, const struct yuxinT_data, base);
	long pos[DIMS];
	memset(pos, 0, DIMS * sizeof(long));
	...
	md_copy_block(DIMS, pos, data->img_dims, dst, data->img_dims2, src, CFL_SIZE);
	md_zaxpy2(DIMS, data->img_dims2, data->strs_img1, dst, 1, data->strs_img2, src); // can be accelerated
}

static void yuxinT0_normal(const linop_data_t* _data, complex float* dst, const complex float* src)
{
	const struct yuxinT_data* data = CONTAINER_OF(_data, const struct yuxinT_data, base);
	complex float* tmp = md_alloc_sameplace(data->N, data->img_dims2, CFL_SIZE, dst);
	yuxinT0_apply(_data, tmp, src);
	yuxinT0_adjoint(_data, dst, tmp);
	md_free(tmp);
}

static void yuxinT1_apply(const linop_data_t* _data, complex float* dst, const complex float* src)
{
	const struct yuxinT_data* data = CONTAINER_OF(_data, const struct yuxinT_data, base);
	long pos[DIMS];
	memset(pos, 0, DIMS * sizeof(long));
...
static void yuxinT1_adjoint(const linop_data_t* _data, complex float* dst, const complex float* src)
{
	const struct yuxinT_data* data = CONTAINER_OF(_data, const struct yuxinT_data, base);
	long pos[DIMS];
	memset(pos, 0, DIMS * sizeof(long));
...
static void yuxinT1_normal(const linop_data_t* _data, complex float* dst, const complex float* src)
{
	const struct yuxinT_data* data = CONTAINER_OF(_data, const struct yuxinT_data, base);
	complex float* tmp = md_alloc_sameplace(data->N, data->img_dims2, CFL_SIZE, dst);
	yuxinT1_apply(_data, tmp, src);
	yuxinT1_adjoint(_data, dst, tmp);
	md_free(tmp);
}

static void yuxinT2_apply(const linop_data_t* _data, complex float* dst, const complex float* src)
{
	const struct yuxinT_data* data = CONTAINER_OF(_data, const struct yuxinT_data, base);
	long pos[DIMS];
	memset(pos, 0, DIMS * sizeof(long));
	md_copy_block(DIMS, pos, data->img_dims1, dst, data->img_dims, src, CFL_SIZE);
}

static void yuxinT2_adjoint(const linop_data_t* _data, complex float* dst, const complex float* src)
{
	const struct yuxinT_data* data = CONTAINER_OF(_data, const struct yuxinT_data, base);
	long pos[DIMS];
	memset(pos, 0, DIMS * sizeof(long));
	md_clear(DIMS, data->img_dims, dst, CFL_SIZE);
	md_copy_block(DIMS, pos, data->img_dims, dst, data->img_dims1, src, CFL_SIZE);
}

static void yuxinT2_normal(const linop_data_t* _data, complex float* dst, const complex float* src)
{
	const struct yuxinT_data* data = CONTAINER_OF(_data, const struct yuxinT_data, base);
	complex float* tmp = md_alloc_sameplace(data->N, data->img_dims1, CFL_SIZE, dst);
	yuxinT2_apply(_data, tmp, src);
	yuxinT2_adjoint(_data, dst, tmp);
	md_free(tmp);
}


struct linop_s* linop_yuxinT0_create(unsigned int N, const long img_dims[N],const long img_dims1[N],const long img_dims2[N])
{
	PTR_ALLOC(struct yuxinT_data, data);
	data->N = N;
	md_copy_dims(N, data->img_dims, img_dims);
	md_copy_dims(N, data->img_dims1, img_dims1);
	md_copy_dims(N, data->img_dims2, img_dims2);

	md_calc_strides(N, data->strs_img, data->img_dims, CFL_SIZE);
	md_calc_strides(N, data->strs_img1, data->img_dims1, CFL_SIZE);
	md_calc_strides(N, data->strs_img2, data->img_dims2, CFL_SIZE);

	return linop_create(N, img_dims2, N, img_dims, &data->base, yuxinT0_apply, yuxinT0_adjoint, yuxinT0_normal, NULL, yuxinT_free_data);
}



struct linop_s* linop_yuxinT1_create(unsigned int N, const long img_dims[N],const long img_dims1[N],const long img_dims2[N])
{
	PTR_ALLOC(struct yuxinT_data, data);
	data->N = N;
	md_copy_dims(N, data->img_dims, img_dims);
	...
	return linop_create(N, img_dims2, N, img_dims, &data->base, yuxinT1_apply, yuxinT1_adjoint, yuxinT1_normal, NULL, yuxinT_free_data);
}


struct linop_s* linop_yuxinT2_create(unsigned int N, const long img_dims[N],const long img_dims1[N],const long img_dims2[N])
{
	PTR_ALLOC(struct yuxinT_data, data);
	data->N = N;
	md_copy_dims(N, data->img_dims, img_dims);
	...
	return linop_create(N, img_dims2, N, img_dims, &data->base, yuxinT2_apply, yuxinT2_adjoint, yuxinT2_normal, NULL, yuxinT_free_data);
}
