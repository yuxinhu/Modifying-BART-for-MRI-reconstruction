...
static void help_reg(void)
{
	printf( "Generalized regularization options (experimental)\n\n"
		...
		"-R Y:7:7:0.3\\t twoc-model\n"
		"-R X:C \t twoc-model\n"
	);
}
// yuxinh: adding new constraints
...
struct reg_s {
	...
	enum { L1WAV, TV, LLR, MLR, IMAGL1, IMAGL2, L1IMG, L2IMG, LLR1, L11 } xform; //yuxinh: adding new constraint
...
static bool opt_reg(void* ptr, char c, const char* optarg)
{
	...
	case 'R': {
		...
		else if (strcmp(rt,"X") == 0) {
			// Yuxin: for the new constraint (sparse in L + S model )
			int ret = sscanf(optarg, "%*[^:]:%f", &regs[r].lambda);
			assert(1 == ret);
			regs[r].xform = L11;
			regs[r].xflags = 7u;
		}
		else if (strcmp(rt,"Y") == 0) {
			// Yuxin: for the new constraint (low-rank in L + S model )
			regs[r].xform = LLR1;
			int ret = sscanf(optarg, "%*[^:]:%d:%d:%f", &regs[r].xflags, &regs[r].jflags, &regs[r].lambda);
			assert(3 == ret);
		}
...
int main_pics(int argc, char* argv[])
{
	...
	long img_dims1[DIMS];
	long img_dims2[DIMS]; //yuxinh: declare a new output dimension for the output image, since for L + S model, the output dimension will change (add by 1), and we put this extra channel along the time dimension (the 10th).
	...
	md_copy_dims(DIMS,img_dims1,img_dims);
	md_copy_dims(DIMS,img_dims2,img_dims);
	img_dims1[10] = 1; 
	...
	if (NULL != pat_file) {
		...
		md_select_dims(DIMS, ~MAPS_FLAG, pat_dims, ksp_dims);
		pattern = md_alloc(DIMS, pat_dims, CFL_SIZE);
		estimate_pattern(DIMS, ksp_dims, MAPS_DIM, pattern, kspace);  //yuxinh
	...
	if ((NULL != traj_file) && (NULL == pat_file)) {
		...
		//md_free(pattern);
		//pattern = NULL;
		//nuconf.toeplitz = true;  // Yuxin Hu 20180516, for Jianmin's nuFFT
	...
	for (int nr = 0; nr < nr_penalties; nr++) {
		...
		case L2IMG:
			...
			break;
		case L11: //yuxin
			img_dims[10] = max_dims[10] + 1;
			
			
			const struct linop_s* temp = linop_yuxinT0_create(DIMS, img_dims, img_dims1, img_dims2); 
			linop_free(temp);
			// yuxinh: when using L+S demols, the forward operator would change (adding our T0 operator)

			
			debug_printf(DP_INFO, "two-c l1-wavelet regularization: %f\n", regs[nr].lambda);

			long minsize2[DIMS] = { [0 ... DIMS - 1] = 1 };
			minsize2[0] = MIN(img_dims[0], 16);
			minsize2[1] = MIN(img_dims[1], 16);
			minsize2[2] = MIN(img_dims[2], 16);
			
			trafos[nr] = linop_yuxinT2_create(DIMS, img_dims, img_dims1, img_dims2); // fix it yuxinh
			// yuxinh: this is the linear transform for the sparse regularization term in the L+S model


			if (7 == regs[nr].xflags) {
				thresh_ops[nr] = prox_wavethresh_create(DIMS, img_dims2, FFT_FLAGS, minsize2, regs[nr].lambda, randshift, use_gpu);
//yuxin
			} else {
				...
				for (unsigned int i = 0; i < DIMS; i++) {
					...
					if ((1 < img_dims[i]) && MD_IS_SET(regs[nr].xflags, i)) {
						...
						wflags = MD_SET(wflags, i);
						minsize2[i] = MIN(img_dims[i], 16);
					}
				}

				thresh_ops[nr] = prox_wavelet3_thresh_create(DIMS, img_dims2, wflags, minsize2, regs[nr].lambda, randshift);
			}

			break;
		case LLR1: //yuxin
			debug_printf(DP_INFO, "Tow-component lowrank regularization: %f\n", regs[nr].lambda);

			// add locally lowrank penalty
			levels = llr_blkdims(blkdims, regs[nr].jflags, img_dims2, llr_blk); //yuxin 
			// change later yuxin
			//debug_printf(DP_INFO, "%d %d %d %d %d %d %d %d %d %d %d\n", img_dims[0],img_dims[1],img_dims[2],img_dims[3],img_dims[4],img_dims[5],img_dims[6],img_dims[7],img_dims[8],img_dims[9],img_dims[10]);
			//debug_printf(DP_INFO, "%d %d %d %d %d %d %d %d %d %d %d\n", img_dims1[0],img_dims1[1],img_dims1[2],img_dims1[3],img_dims1[4],img_dims1[5],img_dims1[6],img_dims1[7],img_dims1[8],img_dims1[9],img_dims1[10]);
			//debug_printf(DP_INFO, "%d %d %d %d %d %d %d %d %d %d %d\n", img_dims2[0],img_dims2[1],img_dims2[2],img_dims2[3],img_dims2[4],img_dims2[5],img_dims2[6],img_dims2[7],img_dims2[8],img_dims2[9],img_dims2[10]);

			assert(1 == levels);
			img_dims[LEVEL_DIM] = levels;
			img_dims1[LEVEL_DIM] = levels;
			img_dims2[LEVEL_DIM] = levels;
			...
			int remove_mean2 = 0;
			//debug_printf(DP_INFO, "yuxin1\n");

			trafos[nr] = linop_yuxinT1_create(DIMS, img_dims, img_dims1, img_dims2); // fix it yuxinh
			//yuxinh: this is the linear transform for the sparse regularization term in the L+S model

			//trafos[nr] = linop_identity_create(DIMS, img_dims2);
			//debug_printf(DP_INFO, "yuxin2\n");
			thresh_ops[nr] = lrthresh_create(img_dims2, randshift, regs[nr].xflags, (const long (*)[DIMS])blkdims, regs[nr].lambda, false, remove_mean2, use_gpu);
			//debug_printf(DP_INFO, "yuxin3\n");

			break;
		}

	}
	// when using two components, notice that L11 constraint should be added first in the input (yuxin)
...
//		mmconf.dynamic_rho = true;
	...
	if (use_gpu) 
#ifdef USE_CUDA
		sense_recon2_gpu(&conf, max_dims, img_dims,image, forward_op, pat_dims, pattern,
				 italgo, iconf, nr_penalties, thresh_ops,
				 (ADMM == algo) ? trafos : NULL, ksp_dims, kspace, image_truth, precond_op);
	//yuxin adding img_dims as input
#else
	assert(0);
#endif
	else
		sense_recon2(&conf, max_dims, img_dims, image, forward_op, pat_dims, pattern,
			     italgo, iconf, nr_penalties, thresh_ops,
			     (ADMM == algo) ? trafos : NULL, ksp_dims, kspace, image_truth, precond_op);
	//yuxinh, adding img_dims as input
